...
public:
    ...
    board_t execute_move_0(board_t board) {
        board_t ret = board;
     ...
     board_t execute_move_1(board_t board) {
        board_t ret = board;
        board_t t = transpose(board);
    ...
    board_t execute_move_2(board_t board) {
        board_t ret = board;
     ...
     board_t execute_move_3(board_t board) {
        board_t ret = board;
   ...
   float _score_toplevel_move(eval_state& state, board_t board, int move) {
    ...
    board_t initial_board(int** arr) {
        /*board_t board = draw_tile() << (4 * unif_random(16));
        return insert_tile_rand(board, draw_tile());*/
        board_t board = mergeIntArrayToInt64(arr);
        return board;
    }


    int play_game(int **arr) {
        int **new_arr= LogToBoard(arr);
        board_t board = initial_board(new_arr);
        static int moveno = 0;
        static int scorepenalty = 0; // "penalty" for obtaining free 4 tiles
    ...
    board_t mergeIntArrayToInt64(int** arr) {
            uint64_t result = 0;

            for (int row = 0; row < 4; ++row) {
                for (int col = 0; col < 4; ++col) {
                    int value = arr[row][col];
                    uint64_t shiftedValue = static_cast<uint64_t>(value) << (row * 16 + col * 4);
                    result |= shiftedValue;
                }
            }
    ...
    int** LogToBoard(int **arr) {
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                if (arr[i][j] != 0) {
                    arr[i][j] = std::log(arr[i][j]) / std::log(2);
                }
            }
        }
        return arr;
